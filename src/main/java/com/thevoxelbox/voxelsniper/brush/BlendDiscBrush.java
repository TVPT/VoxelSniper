    ...
    protected final void blend(final SnipeData v)
    {
        ...
        for (int _x = 0; _x <= _twoBrushSize; _x++)
        {
            for (int _z = 0; _z <= _twoBrushSize; _z++)
            {
                ...
                {
                    if (_materialFrequency[_i] > _modeMatCount && !(this.excludeAir && _i == Material.AIR.getId()) && !(this.excludeWater && (_i == Material.WATER.getId() || _i == Material.STATIONARY_WATER.getId())))
                    {
                ...
                {
                    if (_materialFrequency[_i] == _modeMatCount && !(this.excludeAir && _i == Material.AIR.getId()) && !(excludeWater && (_i == Material.WATER.getId() || _i == Material.STATIONARY_WATER.getId())))
                    {
                        _tiecheck = false;
                    }
                }
        ...
        for (int _x = _twoBrushSize; _x >= 0; _x--)
        {
            ...
            for (int _z = _twoBrushSize; _z >= 0; _z--)
            {
                ...
                {
                    if (!(this.excludeAir && _newMats[_x][_z] == Material.AIR.getId()) && !(this.excludeWater && (_newMats[_x][_z] == Material.WATER.getId() || _newMats[_x][_z] == Material.STATIONARY_WATER.getId())))
                    {
