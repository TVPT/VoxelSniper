...
public class SetRedstoneFlipBrush extends Brush
{
    ...
    public final void parameters(final String[] par, final SnipeData v)
    {
        for (int i = 1; i < par.length; i++)
        {
            ...
            {
                v.sendMessage(TextColors.GOLD + "Set Repeater Flip Parameters:");
                v.sendMessage(TextColors.AQUA + "/b setrf <direction> -- valid direction inputs are(n,s,e,world), Set the direction that you wish to flip your repeaters, defaults to north/south.");
                return;
            }
            if (par[i].startsWith("n") || par[i].startsWith("s") || par[i].startsWith("ns"))
            {
                this.northSouth = true;
                v.sendMessage(TextColors.AQUA + "Flip direction set to north/south");
            }
            else if (par[i].startsWith("e") || par[i].startsWith("world") || par[i].startsWith("ew"))
            {
                this.northSouth = false;
                v.sendMessage(TextColors.AQUA + "Flip direction set to east/west.");
            }
            else
            {
