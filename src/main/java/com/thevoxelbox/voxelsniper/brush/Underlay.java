...
public class Underlay extends PerformBrush {  // You Really Fucked up. Please use a Compiler with syntax highlighting, so that you know when there is a million errors due to a missing bracket.
    ...
    public void parameters(String[] par, com.thevoxelbox.voxelsniper.vData v) {
        if (par[1].equalsIgnoreCase("info")) {
            v.sendMessage(ChatColor.GOLD + "Reverse Overlay brush parameters:");
            v.sendMessage(ChatColor.AQUA + "d[number] (ex: d3) The number of blocks thick to change.");
            v.sendMessage(ChatColor.BLUE + "all (ex: /b reover all) Sets the brush to affect ALL materials");
            if (depth < 1) {
                depth = 1;
            }
            return;
        ...
        for (int x = 1; x < par.length; x++) {
            if (par[x].startsWith("d")) {
                ...
                allBlocks = true;
                v.sendMessage(ChatColor.BLUE + "Will underlay over any block." + depth);
                continue;
            } else if (par[x].startsWith("some")) {
                allBlocks = false;
                v.sendMessage(ChatColor.BLUE + "Will underlay only natural block types." + depth);
                continue;
            } else {
