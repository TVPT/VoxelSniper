    ...
    protected final void oceanator(final SnipeData v, final Undo undo) {
		...
		for (int _x = _minX; _x <= _maxX; _x++) {
			for (int _z = _minZ; _z <= _maxZ; _z++) {
				...
				final int _newSeaFloorLevel = ((this.waterLevel - _wLevelDiff) >= LOW_CUT_LEVEL) ? this.waterLevel - _wLevelDiff : LOW_CUT_LEVEL;
		...
		for (int _i = 0; _i < par.length; _i++) {
			...
			try {
				if (_param.equalsIgnoreCase("info")) {
					...
					v.sendMessage(ChatColor.GREEN + "-cfloor [y|n]  " + ChatColor.BLUE
