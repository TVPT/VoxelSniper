 ...
 * by an amount proportional to the volume of a snipe selection area / the number of blocks touching air in the selection. This is because every solid block
 * surrounded by others should take equally long to check and not change as it would take MC to change them and then check and find no lighting to update. For
 * air blocks surrounded by other air blocks, this brush saves about 80-100 checks by not updating them or their lighting. And for air blocks touching solids,
 ...
 * touching a solid negates one air block floating in air. Thus, for selections that have more air blocks surrounded by air than air blocks touching solids,
 ...
 * should only slow down by the square of the brush size, not the cube of the brush size). For typical terrain, by my calculations, overall speed increase is
        ...
        for (int _z = -v.brushSize; _z <= v.brushSize; _z++) {
            for (int _x = -v.brushSize; _x <= v.brushSize; _x++) {
                for (int _y = -v.brushSize; _y <= v.brushSize; _y++) {
                    Block _block = this.w.getBlockAt(this.bx + _x, this.by + _y, this.bz + _z);
                    if (BlockResetBrushSurface.DENIED_UPDATES.contains(_block.getType())) {
                        continue;
                    }
                    byte _oldData;
                    boolean airFound = false;
                    if (this.w.getBlockAt(this.bx + _x + 1, this.by + _y, this.bz + _z).getTypeId() == 0) {
                        _block = this.w.getBlockAt(this.bx + _x + 1, this.by + _y, this.bz + _z);
                    ...
                    if (this.w.getBlockAt(this.bx + _x, this.by + _y + 1, this.bz + _z).getTypeId() == 0) {
                        _block = this.w.getBlockAt(this.bx + _x, this.by + _y + 1, this.bz + _z);
                        _oldData = _block.getData();
                        _block.setTypeIdAndData(_block.getTypeId(), (byte) ((_block.getData() + 1) & 0xf), true);
                        _block.setTypeIdAndData(_block.getTypeId(), _oldData, true);
                        airFound = true;
                    ...
                    if (this.w.getBlockAt(this.bx + _x, this.by + _y, this.bz + _z + 1).getTypeId() == 0) {
                        _block = this.w.getBlockAt(this.bx + _x, this.by + _y, this.bz + _z + 1);
                        _oldData = _block.getData();
                        _block.setTypeIdAndData(_block.getTypeId(), (byte) ((_block.getData() + 1) & 0xf), true);
                        _block.setTypeIdAndData(_block.getTypeId(), _oldData, true);
                        airFound = true;
                    ...
                    if (airFound) {
                        _block = this.w.getBlockAt(this.bx + _x, this.by + _y, this.bz + _z);
                        _oldData = _block.getData();
                        _block.setTypeIdAndData(_block.getTypeId(), (byte) ((_block.getData() + 1) & 0xf), true);
                        _block.setTypeIdAndData(_block.getTypeId(), _oldData, true);
                    }
