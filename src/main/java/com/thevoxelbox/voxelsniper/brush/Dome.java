...
public class Dome extends Brush {
    ...
    public void parameters(String[] par, com.thevoxelbox.voxelsniper.vData v) {
        if (par[1].equalsIgnoreCase("info")) {
            ...
            v.sendMessage(ChatColor.AQUA + "/b dome h[number] -- set a custom dome height.  Default is the radius of the brush.  Anything else will make it a parabolic dome with circular base.  Cannot be negative.");
            v.sendMessage(ChatColor.BLUE + "/b dome acc [or inacc] -- set brush to half (acc) or full step (inacc) accuracy.  if /v is anything other than 44, will override you and force full step accuracy.");
            return;
        }
        for (int x = 1; x < par.length; x++) {
            if (par[x].startsWith("h")) {
                height = Double.parseDouble(par[x].replace("h", ""));
                v.sendMessage(ChatColor.AQUA + "Dome height set to: " + height);
                continue;
