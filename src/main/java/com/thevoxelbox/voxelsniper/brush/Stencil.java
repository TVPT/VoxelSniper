...
public class Stencil extends Brush {
    ...
    public void arrow(vSniper v) {  //will be used to copy/save later on?
        if (point == 1) {
            //if (v.p.isSneaking()) {
            //    v.p.sendMessage(ChatColor.RED + "Must select at least two corners (reference point optional).");
            //} else {
            firstpoint[0] = tb.getX();
            firstpoint[1] = tb.getZ();
            firstpoint[2] = tb.getY();
            v.p.sendMessage(ChatColor.GRAY + "First point");
            v.p.sendMessage("X:" + firstpoint[0] + " Z:" + firstpoint[1] + " Y:" + firstpoint[2]);
            point = 2;
            //}
        } else if (point == 2) {
            //if (v.p.isSneaking()) {
            ...
            v.p.sendMessage("X:" + secondpoint[0] + " Z:" + secondpoint[1] + " Y:" + secondpoint[2]);
            point = 3;
            //}
        } else if (point == 3) {
            //if (v.p.isSneaking()) {
            ...
            //} else {
                ...
                v.p.sendMessage(ChatColor.GRAY + "Paste Reference point");
                v.p.sendMessage("X:" + pastepoint[0] + " Z:" + pastepoint[1] + " Y:" + pastepoint[2]);
                point = 1;
   ...
   @Override
    public void parameters(String[] par, vSniper v) {
        if (par[1].equalsIgnoreCase("info")) {
            ...
            try{
                ...
                if (f.exists()){
                    v.p.sendMessage(ChatColor.RED + "Stencil '" + Filename + "' exists and was loaded.  Make sure you are using powder if you do not want any chance of overwriting the file.");
                } else {
                    v.p.sendMessage(ChatColor.AQUA + "Stencil '" + Filename + "' does not exist.  Ready to be saved to, but cannot be pasted.");
                }
            }catch (Exception e){
    ...
    public void stencilpaste(vSniper v) {
        if (Filename.matches("NoFileLoaded")){
            v.p.sendMessage(ChatColor.RED + "You did not specify a filename.  This is required.");
                    return;
        }
    ...
    public void stencilsave(vSniper v, boolean reference) {
        ...
        try {
            ...
            v.p.sendMessage(ChatColor.AQUA + "Volume: " + X*Z*Y + " bx:" + bx + " bz:" + bz + " by:" + by);
            ...
            for (int y = 0; y < Y; y++) {
                for (int z = 0; z < Z; z++) {
                    for (int x = 0; x < X; x++) {
                        //v.p.sendMessage("should be checking a block at " + (bx + x) + " " + (by + y) + " " + (bz + z));
                        thisid = (byte) (w.getBlockTypeIdAt(bx + x, by + y, bz + z) - 128);
                        //if (thisid > 0){v.p.sendMessage("high id saved. coords:" + x + "," + y + "," + z);}
                        thisdata = (byte) (clampY(bx + x, by + y, bz + z).getData() - 128);
                        if (thisid != lastid || thisdata != lastdata || counter == 255) {
                            //v.p.sendMessage("ended a run" + " thisid:" + thisid + " thisdata:" + thisdata + " lastid:" + lastid + " lastdata:" + lastdata);
            ...
            //v.p.sendMessage("number of runs = " + arrayIndex);
            ...
            v.p.sendMessage(ChatColor.BLUE + "Saved as '" + Filename + "'.");
            out.close();
