...
public class SplatterBallBrush extends PerformBrush
{
    ...
    private void splatterBall(final SnipeData v, Block targetBlock)
    {
        ...
        {
            v.owner().getPlayer().sendMessage(TextColors.BLUE + "Seed percent set to: 10%");
            this.seedPercent = SEED_PERCENT_DEFAULT;
        }
        if (this.growPercent < GROW_PERCENT_MIN || this.growPercent > GROW_PERCENT_MAX)
        {
            v.owner().getPlayer().sendMessage(TextColors.BLUE + "Growth percent set to: 10%");
            this.growPercent = GROW_PERCENT_DEFAULT;
        }
        if (this.splatterRecursions < SPLATREC_PERCENT_MIN || this.splatterRecursions > SPLATREC_PERCENT_MAX)
        {
            v.owner().getPlayer().sendMessage(TextColors.BLUE + "Recursions set to: 3");
