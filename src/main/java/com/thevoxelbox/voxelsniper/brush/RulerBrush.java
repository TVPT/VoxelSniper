...
public class RulerBrush extends Brush
{
    ...
    protected final void powder(final SnipeData v)
    {
        ...
        {
            v.sendMessage(TextColors.RED + "Warning: You did not select a first coordinate with the arrow. Comparing to point 0,0,0 instead.");
            return;
        }

        v.sendMessage(TextColors.BLUE + "Format = (second coord - first coord)");
        v.sendMessage(TextColors.AQUA + "X change: " + (this.getTargetBlock().getX() - this.coords.getX()));
        v.sendMessage(TextColors.AQUA + "Y change: " + (this.getTargetBlock().getY() - this.coords.getY()));
        v.sendMessage(TextColors.AQUA + "Z change: " + (this.getTargetBlock().getZ() - this.coords.getZ()));
        ...
        v.sendMessage(TextColors.AQUA + "Euclidean distance = " + distance);
        v.sendMessage(TextColors.AQUA + "Block distance = " + blockDistance);
    }
    ...
    public final void parameters(final String[] par, final SnipeData v)
    {
        for (int i = 1; i < par.length; i++)
        {
            ...
            {
                v.sendMessage(TextColors.GOLD + "Ruler Brush instructions: Right click first point with the arrow. Right click with powder for distances from that block (can repeat without getting a new first block.) For placing blocks, use arrow and input the desired coordinates with parameters.");
                v.sendMessage(TextColors.LIGHT_PURPLE + "/b r x[x value] y[y value] z[z value] -- Will place blocks one at a time of the type you have set with /v at the location you click + this many units away.  If you don't include a value, it will be zero.  Don't include ANY values, and the brush will just measure distance.");
