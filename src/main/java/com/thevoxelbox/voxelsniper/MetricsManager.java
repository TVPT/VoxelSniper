	...
	public void start() {
		try {
			...
			_graph.addPlotter(new Metrics.Plotter("Snipers Online") {
				...
				public int getValue() {
					int _count = 0;
					for (Player _player : Bukkit.getOnlinePlayers()) { // hackaround
						if (_player.hasPermission("voxelsniper.sniper.punish")) {
							_count++;
						}
					}
					return _count;
			...
			_graph.addPlotter(new Metrics.Plotter("Litesnipers Online") {
				...
				public int getValue() {
					int _count = 0;
					for (Player _player : Bukkit.getOnlinePlayers()) { // hackaround
						if (_player.hasPermission("voxelsniper.litesniper.snipe")) {
							_count++;
						}
					}
					return _count;
